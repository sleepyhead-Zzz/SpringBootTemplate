...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public RoleInfo getRoleInfo(Long roleId, boolean isAdmin) {
        ...
        if (isAdmin) {
            ...
            menuQuery.select(SysMenuEntity::getMenu_id);
            List<SysMenuEntity> allMenus = menuService.list(menuQuery);

            Set<Long> allMenuIds = allMenus.stream().map(SysMenuEntity::getMenu_id).collect(Collectors.toSet());
        ...
        Set<Long> menuIds = menuList.stream().map(SysMenuEntity::getMenu_id).collect(Collectors.toSet());
        ...
        Set<Long> deptIdSet = CollUtil.newHashSet();
